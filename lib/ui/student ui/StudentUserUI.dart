...
class StudentUserUI extends StatefulWidget{
  @override
  State<StatefulWidget> createState() {
    return _StudentUserUI();
  }
}

class _StudentUserUI extends State<StudentUserUI> {
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.start,
      ...
      children: [
        ...
        Padding(padding: EdgeInsets.only(left: W(5) , right: W(5) , top: H(24) ,bottom: H(6.5)) ,
            child: Text(glb.student.name , style: GoogleFonts.varelaRound(fontWeight: FontWeight.bold , fontSize: W(6)) ,
            )),

        Padding(padding: EdgeInsets.only(left: W(5) , right: W(5) , bottom: H(2)) ,
            child: Text(glb.student.schoolName , style: GoogleFonts.varelaRound( fontSize: W(3.5) )
            )),

        Padding(padding: EdgeInsets.only(left: W(5) , right: W(5) , bottom: H(2) ) ,
            child: Text("Roll Num : " + glb.student.rollNum , style: GoogleFonts.varelaRound( fontSize: W(3.4) ) )),
  ...
  Widget signOutButton (BuildContext context) {
    return Container( width: W(30), height: H(8),
      child: ElevatedButton(
        ...
        onPressed: () async {
          unSubscribeFromTopic(glb.student.subjectList);
          glb.student.destroy();
  ...
  Widget resetButton(context){
    return GestureDetector(
      ...
      child: Text("Reset Password" ,
        style: TextStyle(color: Colors.blueAccent , fontSize: W(4),
    ...
    return AlertDialog(
      ...
      actions: [
        Padding(padding: EdgeInsets.only(right: W(4))  ,
          child: ElevatedButton(
              style: ButtonStyle(backgroundColor: MaterialStateProperty.all<Color>( CupertinoColors.activeBlue )),
              child: Text("Ok" , style: GoogleFonts.varela(),),
              onPressed: (){
                Navigator.pushAndRemoveUntil(context, CupertinoPageRoute(
                    builder: (context) =>  LoginUI() ) , (route) => false );
              }),)
